...
# 1) Account page: real Firebase account UI.
...
if old not in s: raise SystemExit('app init marker missing')
s=s.replace(old,new,1)

# 4) Account state vars.
...
# 7) Order creation: bind to authenticated account and tracking.
...
if old not in s: raise SystemExit('order create marker missing')
s=s.replace(old,new,1)

# Local order cache retains uid too.
...
if old not in s: raise SystemExit('local order marker missing')
s=s.replace(old,new,1)

p.write_text(s)
print('customer account patch applied')
